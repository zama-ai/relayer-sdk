...
/**
 * Recursively converts Uint8Array values to hex strings in an object.
 * viem requires bytes fields as hex strings, but ethers produces Uint8Array.
 */
function convertBytesToHex(value: unknown): unknown {
  if (value instanceof Uint8Array) {
    return bytesToHex(value);
  }
  if (Array.isArray(value)) {
    return value.map(convertBytesToHex);
  }
...
export async function recoverTypedDataAddress(
  ...
  // viem requires bytes values as hex strings, convert any Uint8Array
  const viemMessage = convertBytesToHex(message) as Record<string, unknown>;
