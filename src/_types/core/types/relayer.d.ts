...
export type RelayerKeyUrlOptions = Prettify<Omit<RelayerCommonOptions, "timeout"> & {
    onProgress?: (args: RelayerKeyUrlProgressArgs) => void;
}>;
export type RelayerInputProofOptions = Prettify<RelayerCommonOptions & {
    onProgress?: (args: RelayerInputProofProgressArgs) => void;
}>;
export type RelayerUserDecryptOptions = Prettify<RelayerCommonOptions & {
    onProgress?: (args: RelayerUserDecryptProgressArgs) => void;
}>;
export type RelayerDelegatedUserDecryptOptions = Prettify<RelayerCommonOptions & {
    onProgress?: (args: RelayerDelegatedUserDecryptProgressArgs) => void;
}>;
export type RelayerPublicDecryptOptions = Prettify<RelayerCommonOptions & {
...
export type FetchResultOf<O extends RelayerPostOperation> = O extends "INPUT_PROOF" ? FetchInputProofResult : O extends "PUBLIC_DECRYPT" ? FetchPublicDecryptResult : FetchUserDecryptResult;
export type RelayerProgressArgs<O extends RelayerPostOperation> = RelayerProgressQueued<O> | RelayerProgressThrottled<O> | RelayerProgressSucceeded<O> | RelayerProgressTimeout<O> | RelayerProgressAbort<O> | RelayerProgressFailed<O>;
...
export type RelayerProgressStatusBase<T extends RelayerProgressTypeValue, O extends RelayerPostOperation, S extends RelayerSuccessStatus | RelayerFailureStatus> = Prettify<RelayerProgressBase<T, O> & {
    readonly method: "POST" | "GET";
    readonly status: S;
}>;
export type RelayerProgressJobIdBase<T extends RelayerProgressTypeValue, O extends RelayerPostOperation, S extends RelayerSuccessStatus | RelayerFailureStatus> = Prettify<RelayerProgressStatusBase<T, O, S> & {
    readonly jobId: string;
}>;
export type RelayerProgressQueued<O extends RelayerPostOperation> = Prettify<RelayerProgressJobIdBase<"queued", O, 202> & {
    readonly requestId: string;
    readonly retryAfterMs: number;
    readonly elapsed: number;
}>;
export type RelayerProgressThrottled<O extends RelayerPostOperation> = Prettify<RelayerProgressStatusBase<"throttled", O, 429> & {
...
export type RelayerProgressSucceeded<O extends RelayerPostOperation> = Prettify<RelayerProgressJobIdBase<"succeeded", O, 200> & {
    readonly requestId: string;
    readonly elapsed: number;
    readonly result: FetchResultOf<O>;
}>;
export type RelayerProgressFailed<O extends RelayerPostOperation, S extends RelayerFailureStatus = RelayerFailureStatus> = Prettify<RelayerProgressStatusBase<"failed", O, S> & {
    readonly elapsed: number;
    readonly relayerApiError: {
        readonly label: string;
        readonly message: string;
