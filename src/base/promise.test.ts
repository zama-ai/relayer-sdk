...
// =================
//
// npx jest --colors --passWithNoTests ./src/base/promise.test.ts
//
////////////////////////////////////////////////////////////////////////////////

describe('executeWithBatching', () => {
  it.each([true, false])(
    'executes all factories (parallel=%s)',
    async (parallel) => {
  ...
  it.each([true, false])(
    'handles empty array (parallel=%s)',
    async (parallel) => {
  ...
  it.each([true, false])(
    'handles single factory (parallel=%s)',
    async (parallel) => {
  ...
  it.each([true, false])(
    'propagates single error (parallel=%s)',
    async (parallel) => {
      const factories: PromiseFactory<number>[] = [
        async () => 1,
        async () => {
      ...
      await expect(executeWithBatching(factories, parallel)).rejects.toThrow(
        'Factory error',
      );
    },
  );
  ...
  it.each([true, false])(
    'propagates multiple errors (parallel=%s)',
    async (parallel) => {
      const factories: PromiseFactory<number>[] = [
        async () => 1,
        async () => {
  ...
  it.each([
    ...
    'propagates multiple errors with delay (parallel=$parallel)',
    async ({ parallel, expectedError }) => {
      const factories: PromiseFactory<number>[] = [
        async () => 1,
        async () => {
  ...
  it.each([true, false])(
    'preserves types correctly (parallel=%s)',
    async (parallel) => {
      interface User {
        id: number;
        name: string;
  ...
  it.each([true, false])(
    'handles mixed return values (parallel=%s)',
    async (parallel) => {
