...
// Jest Command line
// =================
// npx jest --colors --passWithNoTests ./src/relayer-provider/v2/RelayerV2AsyncRequest.rate_limited.test.ts
...
const RATE_LIMIT_ERROR: RelayerApiError429Type = {
  label: 'rate_limited',
...
const RATE_LIMIT_BODY: RelayerV2ResponseFailed = {
  status: 'failed',
...
describeIfFetchMock('RelayerV2AsyncRequest - Rate Limit (429)', () => {
  ...
  it('v2: rate_limited error triggers onProgress with throttled type', async () => {
    ...
    fetchMock.post(requestUrl, () => {
      ...
      return {
        status: 202,
        headers: { 'Retry-After': '1' },
        body: makeQueuedBody(JOB_ID, 'req-queued-123'),
      };
    });
    ...
    // Verify rate limit progress event was fired
    expect(progressEvents.length).toBe(1);
    expect(progressEvents[0].type).toBe('throttled');
    expect(progressEvents[0].status).toBe(429);
    expect(progressEvents[0].relayerApiError.label).toBe('rate_limited');
    expect(progressEvents[0].relayerApiError.message).toBe('Too many requests');
  ...
  it('v2: multiple consecutive 429 errors are retried', async () => {
    let postFetchCount = 0;
  ...
  it('v2: 429 error respects Retry-After header', async () => {
    let postFetchCount = 0;
