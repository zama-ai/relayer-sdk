...
// =================
//
// npx jest --colors --passWithNoTests ./src/fhevm-base/FheType.test.ts
// npx jest --colors --passWithNoTests ./src/fhevm-base/FheType.test.ts --testNamePattern=xxx
// npx jest --colors --passWithNoTests --coverage ./src/fhevm-base/FheType.test.ts --collectCoverageFrom=./src/fhevm-base/FheType.ts
//
...
describe('FheType', () => {
  ...
  describe('assertIsEncryptionBits', () => {
    ...
    it('throws InvalidTypeError for non-number values', () => {
      ...
      expect(() => assertIsEncryptionBits(undefined, {})).toThrow(
        InvalidTypeError,
      );
    });

    it('includes varName in error when provided', () => {
      expect(() => assertIsEncryptionBits(4, { subject: 'myVar' })).toThrow(
        /myVar/,
      );
    });
  });
  ...
  describe('assertIsEncryptionBitsArray', () => {
    ...
    it('throws InvalidTypeError for non-array values', () => {
      ...
      expect(() => assertIsEncryptionBitsArray(123, {})).toThrow(
        InvalidTypeError,
      );
      expect(() => assertIsEncryptionBitsArray(null, {})).toThrow(
        InvalidTypeError,
      );
    ...
    it('includes varName with index in error when provided', () => {
      expect(() =>
        assertIsEncryptionBitsArray([8, 4], { subject: 'myArray' }),
      ).toThrow(/myArray\[1\]/);
    });

    it('throws without varName when not provided', () => {
      expect(() => assertIsEncryptionBitsArray([4], {})).toThrow(
        InvalidTypeError,
      );
    });
  });
  ...
  describe('fheTypeIdFromEncryptionBits', () => {
    ...
    it('throws FheTypeError for invalid bit widths', () => {
      expect(() =>
        fheTypeIdFromEncryptionBits(asEncryptionBits(4 as any)),
      ).toThrow(InvalidTypeError);
      expect(() =>
        fheTypeIdFromEncryptionBits(asEncryptionBits(4 as any)),
      ).toThrow('expected 2|8|16|32|64|128|160|256');
      expect(() =>
        fheTypeIdFromEncryptionBits(asEncryptionBits(512 as any)),
      ).toThrow(InvalidTypeError);
    });
  });
  ...
  describe('fheTypeIdFromName', () => {
    ...
    it('throws FheTypeError for invalid names', () => {
      expect(() => fheTypeIdFromName(asFheTypeName('euint4' as any))).toThrow(
        InvalidTypeError,
      );
      expect(() => fheTypeIdFromName(asFheTypeName('euint4' as any))).toThrow(
        'expected ebool|euint8|euint16|euint32|euint64|euint128|eaddress|euint256',
      );
      expect(() => fheTypeIdFromName(asFheTypeName('invalid' as any))).toThrow(
        InvalidTypeError,
      );
    });
  });
  ...
  describe('fheTypeNameFromId', () => {
    ...
    it('throws FheTypeError for invalid ids', () => {
      expect(() => fheTypeNameFromId(asFheTypeId(1 as any))).toThrow(
        InvalidTypeError,
      );
      expect(() => fheTypeNameFromId(asFheTypeId(1 as any))).toThrow(
        'expected 0|2|3|4|5|6|7|8',
      );
      expect(() => fheTypeNameFromId(asFheTypeId(9 as any))).toThrow(
        InvalidTypeError,
      );
    });
  });
  ...
  describe('solidityPrimitiveTypeNameFromFheTypeId', () => {
    ...
    it('throws FheTypeError for invalid ids', () => {
      expect(() =>
        solidityPrimitiveTypeNameFromFheTypeId(asFheTypeId(1 as any)),
      ).toThrow(InvalidTypeError);
      expect(() =>
        solidityPrimitiveTypeNameFromFheTypeId(asFheTypeId(1 as any)),
      ).toThrow('expected 0|2|3|4|5|6|7|8');
    });
  });
  ...
  describe('encryptionBitsFromFheTypeId', () => {
    ...
    it('throws FheTypeError for invalid ids', () => {
      expect(() => encryptionBitsFromFheTypeId(asFheTypeId(1 as any))).toThrow(
        InvalidTypeError,
      );
      expect(() => encryptionBitsFromFheTypeId(asFheTypeId(1 as any))).toThrow(
        'expected 0|2|3|4|5|6|7|8',
      );
      expect(() => encryptionBitsFromFheTypeId(asFheTypeId(9 as any))).toThrow(
        InvalidTypeError,
      );
    });
  });
  ...
  describe('encryptionBitsFromFheTypeName', () => {
    ...
    it('throws FheTypeError for invalid names', () => {
      expect(() =>
        encryptionBitsFromFheTypeName(asFheTypeName('euint4' as any)),
      ).toThrow(InvalidTypeError);
      expect(() =>
        encryptionBitsFromFheTypeName(asFheTypeName('euint4' as any)),
      ).toThrow(
      ...
      expect(() =>
        encryptionBitsFromFheTypeName(asFheTypeName('invalid' as any)),
      ).toThrow(InvalidTypeError);
    });
  });
