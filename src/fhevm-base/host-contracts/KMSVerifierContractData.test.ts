...
import { fetchKMSVerifierContractData } from './KMSVerifierContractData';
import {
  FhevmChainClient,
  FhevmLibs,
  NativeClient,
...
import { KMSVerifierContractData } from '../types/public-api';
...
// =================
//
// npx jest --colors --passWithNoTests ./src/fhevm-base/KMSVerifier.test.ts
// npx jest --colors --passWithNoTests ./src/fhevm-base/KMSVerifier.test.ts --testNamePattern=xxx
// npx jest --colors --passWithNoTests --coverage ./src/fhevm-base/KMSVerifier.test.ts --collectCoverageFrom=./src/fhevm-base/KMSVerifier.ts
//
//
// Testnet:
// =======
// npx jest --config jest.testnet.config.cjs --colors --passWithNoTests ./src/fhevm-base/KMSVerifier.test.ts
//
// Devnet:
// =======
// npx jest --config jest.devnet.config.cjs --colors --passWithNoTests ./src/fhevm-base/KMSVerifier.test.ts
//
////////////////////////////////////////////////////////////////////////////////
    ...
    config: {
      hostChainConfig: {
        kmsVerifierContractAddress:
          kmsVerifierContractAddress as ChecksummedAddress,
      },
    },
  };
}
...
describeIfFetchMock('KMSVerifier mock', () => {
  ...
  describe('loadFromChain error cases', () => {
    ...
    it('throws on invalid threshold (too large)', async () => {
      ...
      await expect(fetchKMSVerifierContractData(client)).rejects.toThrow(
        'Invalid KMSVerifier kms signers threshold.',
      );
    });
    ...
    it('throws on invalid kmsSigners (not array)', async () => {
      ...
      await expect(fetchKMSVerifierContractData(client)).rejects.toThrow(
        'Invalid KMSVerifier kms signers addresses.',
      );
    });
    ...
    it('throws on invalid eip712Domain name', async () => {
      ...
      await expect(fetchKMSVerifierContractData(client)).rejects.toThrow(
        'Invalid KMSVerifier EIP-712 domain.',
      );
    });
...
describe('KMSVerifier', () => {
  ...
  describe('loadFromChain', () => {
    let kmsVerifier: KMSVerifierContractData;
    let fhevmLibs: FhevmLibs;
    ...
    beforeAll(async () => {
      ...
      kmsVerifier = await fetchKMSVerifierContractData(client);
    });
