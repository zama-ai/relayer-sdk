...
// // eslint-disable-next-line @typescript-eslint/naming-convention
// declare const __InitInputWasm: unique symbol;
// // eslint-disable-next-line @typescript-eslint/naming-convention
// declare const __InitOutputWasm: unique symbol;
// // eslint-disable-next-line @typescript-eslint/naming-convention
// declare const __TKMSWasm: unique symbol;
...
// type InitInputWasmType = InitInput & {
//   readonly [__InitInputWasm]: never;
// };

// type InitOutputWasmType = InitOutput & {
//   readonly [__InitOutputWasm]: never;
// };
...
class TkmsPublicEncKeyMlKem512Impl {
  ...
  constructor(
    token: symbol,
    publicEncKeyMlKem512Wasm: PublicEncKeyMlKem512WasmType,
  ) {
    verifyToken(token);
  ...
  public static [GET_BYTES_HEX_FUNC](key: unknown, token: symbol): BytesHex {
    ...
    if (key.#bytesHex === undefined) {
      const bytes: Bytes = ml_kem_pke_pk_to_u8vec(
        key.#publicEncKeyMlKem512Wasm,
      );
...
class TkmsPrivateEncKeyMlKem512Impl implements TkmsPrivateKey {
  ...
  constructor(
    token: symbol,
    privateEncKeyMlKem512Wasm: PrivateEncKeyMlKem512WasmType,
  ) {
    verifyToken(token);
  ...
  public static [GET_PUBLIC_KEY_FUNC](
    ...
    if (key.#publicKey === undefined) {
      ...
      key.#publicKey = new TkmsPublicEncKeyMlKem512Impl(
        token,
...
export async function generateTkmsPrivateKey(
  ...
  const privateEncKeyMlKem512Wasm: PrivateEncKeyMlKem512WasmType =
    ml_kem_pke_keygen() as PrivateEncKeyMlKem512WasmType;

  return new TkmsPrivateEncKeyMlKem512Impl(
    PRIVATE_TKMS_LIB_TOKEN,
    privateEncKeyMlKem512Wasm,
  );
}
...
export async function decryptAndReconstruct(
  ...
  const tkmsPublicKey: TkmsPublicEncKeyMlKem512Impl =
  ...
  const privateEncKeyMlKem512Wasm: PrivateEncKeyMlKem512WasmType =
    TkmsPrivateEncKeyMlKem512Impl[GET_NATIVE_FUNC](
      tkmsPrivateKey,
      PRIVATE_TKMS_LIB_TOKEN,
    );

  const publicEncKeyMlKem512Wasm: PublicEncKeyMlKem512WasmType =
    TkmsPublicEncKeyMlKem512Impl[GET_NATIVE_FUNC](
      tkmsPublicKey,
      PRIVATE_TKMS_LIB_TOKEN,
    );

  const publicEncKeyMlKem512WasmBytesHex: BytesHex =
    TkmsPublicEncKeyMlKem512Impl[GET_BYTES_HEX_FUNC](
      tkmsPublicKey,
      PRIVATE_TKMS_LIB_TOKEN,
    );
  ...
  const indexedServerAddressesWasm: ServerIdAddrWasmType[] =
    metadata.kmsSignersContext.signers.map((kmsSigner, index) => {
      const kmsSignerPartyId = index + 1;
      return new_server_id_addr(
        kmsSignerPartyId,
        kmsSigner,
      ) as ServerIdAddrWasmType;
    });

  const clientWasm: ClientWasmType = new_client(
...
export function verifyTkmsPrivateKey(
  ...
  parameters: VerifyTkmsPrivateKeyParameters,
): void {
