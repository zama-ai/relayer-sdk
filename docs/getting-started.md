...
A **client** is your main interface to the SDK. It's bound to a specific chain and provider, and gives you methods like `encrypt()`, `decrypt()`, and `readPublicValue()`.
...
The result contains:
- `encrypted.encryptedInputs` — one encrypted value per input, in the same order you provided them
...
await contract.increment(
  encrypted.encryptedInputs[0],  // → externalEuint32 in Solidity
...
You can encrypt multiple values in a single `encrypt()` call (up to 2048 encrypted bits total). Each value gets its own entry in `encryptedInputs`, in the same order:
...
await contract.myFunction(
  encrypted.encryptedInputs[0],  // externalEuint32
  encrypted.encryptedInputs[1],  // externalEbool
  encrypted.encryptedInputs[2],  // externalEaddress
...
2. **A signed decrypt permit** — an EIP-712 message signed by the user's wallet, authorizing decryption for specific contracts and a time window
...
// 3. Sign the permit with the user's wallet
//    With ethers.js:
...
// 4. Bundle into a reusable signed permit
//    With ethers.js:
const signedPermit = createSignedPermit(permit, signature, await signer.getAddress());
//    With viem:
// const signedPermit = createSignedPermit(permit, signature, walletClient.account.address);

// 5. Decrypt — pass the encrypted value you read from your contract
const encryptedBalance = await contract.balances(userAddress);

const results = await client.decrypt({
  encryptedValues: [
    { encrypted: encryptedBalance, contractAddress: "0xYourContract..." },
  ],
  e2eTransportKeyPair,
  signedPermit,
});
...
Everything else (signing, calling `decrypt()`) is the same. The Zama Protocol verifies that the on-chain ACL grants the signer access to the owner's values.

## Step 5: Read public values

`readPublicValue()` reveals encrypted values that your smart contract has explicitly marked as publicly readable. Anyone can call this — no keys or signatures needed.
...
On the SDK side, read the encrypted value from your contract, then pass it to `readPublicValue()`:

```ts
// Read the encrypted value from the contract
const encryptedTotalSupply = await contract.totalSupply();

const result = await client.readPublicValue([encryptedTotalSupply]);

// Access the plaintext
const value = result.values[0].value;   // the decrypted value
const type = result.values[0].fheType;  // "euint32", "ebool", etc.
```
...
| --- | --- |
| `@fhevm/sdk/ethers` | Client factories + all actions (ethers.js v6 adapter) |
| `@fhevm/sdk/viem` | Client factories + all actions (viem adapter) — identical API |
